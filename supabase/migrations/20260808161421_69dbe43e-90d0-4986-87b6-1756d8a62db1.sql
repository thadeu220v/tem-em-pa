DROP FUNCTION IF EXISTS public.search_companies_autocomplete(text, uuid, integer);

CREATE OR REPLACE FUNCTION public.search_companies_autocomplete(q text, _city_id uuid, lim integer DEFAULT 8)
 RETURNS TABLE(id uuid, name text, slug text, neighborhood text, logo_url text, city_slug text, city_name text, state text)
 LANGUAGE plpgsql
...
DECLARE
  ...
  cap int := LEAST(GREATEST(lim, 1), 120);
  ids uuid[] := ARRAY[]::uuid[];
  tiers int[] := ARRAY[]::int[];
  need int;
BEGIN
  ...
  SELECT array_agg(t.id), array_agg(1) INTO ids, tiers
  FROM (
    SELECT c.id
  ...
  IF need > 0 THEN
    ...
    SELECT ids || array_agg(e.id), tiers || array_agg(2) INTO ids, tiers FROM extra e;
    ids := COALESCE(ids, ARRAY[]::uuid[]);
    tiers := COALESCE(tiers, ARRAY[]::int[]);
    need := cap - COALESCE(array_length(ids,1),0);
  END IF;
  ...
  IF need > 0 THEN
    ...
    SELECT ids || array_agg(e.id ORDER BY e.rnk DESC, e.name ASC),
           tiers || array_agg(3 ORDER BY e.rnk DESC, e.name ASC) INTO ids, tiers FROM extra e;
    ids := COALESCE(ids, ARRAY[]::uuid[]);
    tiers := COALESCE(tiers, ARRAY[]::int[]);
  END IF;
  ...
  SELECT c.id, c.name, c.slug, n.name AS neighborhood, c.logo_url, ci.slug AS city_slug,
         ci.name AS city_name, ci.state AS state
