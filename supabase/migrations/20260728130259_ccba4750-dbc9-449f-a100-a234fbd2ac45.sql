...
BEGIN
  ...
  -- Tier 1: prefix
  SELECT array_agg(t.id ORDER BY t.is_featured DESC, t.name ASC),
         array_agg(1 ORDER BY t.is_featured DESC, t.name ASC)
    INTO ids, tiers
  FROM (
    SELECT c.id, c.name, c.is_featured
  ...
  IF need > 0 THEN
    ...
    SELECT ids || array_agg(e.id ORDER BY e.is_featured DESC, e.name ASC),
           tiers || array_agg(2 ORDER BY e.is_featured DESC, e.name ASC)
      INTO ids, tiers
    FROM extra e;
    ids := COALESCE(ids, ARRAY[]::uuid[]);
    tiers := COALESCE(tiers, ARRAY[]::int[]);
  ...
  IF need > 0 THEN
    WITH extra AS (
      SELECT c.id, c.name, c.is_featured, ts_rank(c.search_tsv, websearch_to_tsquery('portuguese', uq)) AS rnk
      FROM public.companies c
      WHERE c.status = 'approved'
        AND (_city_id IS NULL OR c.city_id = _city_id)
    ...
    SELECT ids || array_agg(e.id ORDER BY e.rnk DESC, e.is_featured DESC, e.name ASC),
           tiers || array_agg(3 ORDER BY e.rnk DESC, e.is_featured DESC, e.name ASC)
      INTO ids, tiers
    FROM extra e;
    ids := COALESCE(ids, ARRAY[]::uuid[]);
    tiers := COALESCE(tiers, ARRAY[]::int[]);
