...
BEGIN
  ...
  -- Tier 1: prefix — order by the indexed expression so we stream from the btree
  SELECT array_agg(t.id ORDER BY t.uname ASC),
         array_agg(1 ORDER BY t.uname ASC)
    INTO ids, tiers
  FROM (
    SELECT c.id, public.immutable_unaccent(lower(c.name)) AS uname
    FROM public.companies c
    WHERE c.status = 'approved'
      AND (_city_id IS NULL OR c.city_id = _city_id)
      AND public.immutable_unaccent(lower(c.name)) LIKE uq || '%'
    ORDER BY public.immutable_unaccent(lower(c.name)) ASC
    LIMIT cap
  ) t;
  ...
  IF need > 0 THEN
    ...
    SELECT ids || (SELECT array_agg(e.id ORDER BY e.uname ASC) FROM (SELECT * FROM extra ORDER BY uname LIMIT need) e),
           tiers || (SELECT array_agg(2) FROM (SELECT 1 FROM extra LIMIT need) x)
      INTO ids, tiers;
    ids := COALESCE(ids, ARRAY[]::uuid[]);
    tiers := COALESCE(tiers, ARRAY[]::int[]);
    need := cap - COALESCE(array_length(ids,1),0);
